...
## Phase 1: Deploying the Monolithic BookStore on AWS

This document details the process of deploying the monolithic BookStore application on an AWS Virtual Machine, with a custom domain, SSL certificate, and NGINX reverse proxy.
...
- [Step 4: SSL Certificate Configuration](#step-4-ssl-certificate-configuration)
...
- One EC2 Amazon Linux 2023 instance
...
git clone [https://github.com/Alkran93/bookstore-monolith.git](https://github.com/Alkran93/bookstore-monolith.git)
...
### CloudFormation Template Customization

The `infra/phase1.yaml` template must be modified to include:
- Your EC2 key pair
...
1. Creation of a security group with ports 22, 80, 443, and 5000 open
2. Launching an EC2 instance with Amazon Linux 2023
3. Installation and configuration of Docker, Docker-Compose, and NGINX
4. Cloning the repository and preparing the application
5. Initial configuration of NGINX as a reverse proxy
...
2. Go to the DNS section for the `proyecto2.shop` domain
...
Execute the SSL configuration script:
...
The script:
1. Verifies the Certbot installation
2. Checks that NGINX is running
3. Obtains and configures an SSL certificate for `proyecto2.shop` and `www.proyecto2.shop`
4. Verifies the status of the Docker application
...
```bash
# Verify that the application responds via HTTPS
curl -vk [https://proyecto2.shop](https://proyecto2.shop)
...
1.  **The application does not respond on port 5000**
    ```bash
    # Verify that the container is running
    docker ps

    # If it's not running, start docker-compose
    cd /app
    docker-compose up -d
...
2.  **NGINX does not redirect correctly to the application**
    ```bash
...
3.  **Error obtaining SSL certificate**
    ```bash
    # Verify that DNS records have propagated
